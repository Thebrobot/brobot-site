...
export default function Navbar({ pathname = "/" }: NavbarProps) {
  ...
  const systems = [
    { name: "Brobot", href: "/", icon: LayoutDashboard, image: "/images/favicon-nav.webp", desc: "MAIN ECOSYSTEM", color: "bg-neutral-950", iconColor: "text-white" },
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center p-4 md:p-8 pointer-events-none">
      {/* Plain nav — no motion.nav entrance animation (was causing LCP delay) */}
      <nav
        className={cn(
          ...
          pathname === "/ai-phone-crm"
            ? isScrolled ? "bg-[#020617] translate-y-2" : "bg-[#020617]"
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 md:gap-3 group px-3 md:px-5 py-2 md:py-3 cursor-pointer bg-white/10 rounded-full border border-white/5 backdrop-blur-md transition-all hover:bg-white/20">
            {/* CSS pulse animation instead of motion.div */}
            <div
              className={cn(
                "w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 relative flex-shrink-0 animate-[hub-pulse_4s_ease-in-out_infinite]",
              ...
              >
                <div className="flex flex-col text-left">
                      ...
                      return (
                        ...
                        >
                          <div className={cn(
                            ...
                            {sys.image ? (
                              <img src={sys.image} alt={sys.name} width={40} height={40} className="w-full h-full object-cover" />
                            ) : (
                              <sys.icon className={cn("w-5 h-5", sys.iconColor)} />
                            )}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-[13px] font-black text-neutral-900 tracking-wider truncate">{sys.name}</div>
                  ...
                  <div className="mt-3 pt-3 border-t border-neutral-100 px-1">
                    <a href="/blog" className="flex items-center gap-3 p-3 rounded-2xl hover:bg-neutral-50 transition-all group mb-2">
                      <div className="w-10 h-10 rounded-xl bg-neutral-100 flex items-center justify-center transition-all shadow-sm">
                        <span className="text-lg">📰</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-[13px] font-black text-neutral-900 tracking-wider truncate">Blog & Insights</div>
                        <div className="text-[10px] text-neutral-900 font-bold uppercase tracking-tight truncate">Latest Updates</div>
                      </div>
                      <ArrowRight className="w-3 h-3 text-neutral-400 group-hover:text-amber-500 group-hover:translate-x-1 transition-all" />
                    </a>
                    <a href="/contact" className="flex items-center gap-3 p-3 rounded-2xl hover:bg-neutral-50 transition-all group mb-2">
                      <div className="w-10 h-10 rounded-xl bg-neutral-100 flex items-center justify-center transition-all shadow-sm">
                        <Mail className="w-5 h-5 text-neutral-600" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-[13px] font-black text-neutral-900 tracking-wider truncate">Contact Us</div>
                        <div className="text-[10px] text-neutral-900 font-bold uppercase tracking-tight truncate">Phone, email & form</div>
                      </div>
                      <ArrowRight className="w-3 h-3 text-neutral-400 group-hover:text-amber-500 group-hover:translate-x-1 transition-all" />
                    </a>
                    <a href="/industries" className="flex items-center justify-between p-3 rounded-2xl bg-amber-600 text-white hover:bg-amber-500 transition-all shadow-lg shadow-amber-600/20 group">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center">
                          <Command className="w-4 h-4 text-white" />
                        </div>
                        <div className="text-[12px] font-black uppercase tracking-wider">Industry Solutions</div>
                      </div>
